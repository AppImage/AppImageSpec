...
[Image format]: #image-format "Specification - Image Format"
  [Version 2.x image format]: #type-2-image-format "Specification - Image Format - Version 2.x image format"
...
Version | Date | Notes
--- | --- | ---
Draft | 2023-06-18 | MUST use static linking for runtime, MUST support TARGET_APPIMAGE, MUST use additional magic bytes
...
An [AppImage] which conforms to the Version 2.x image format:
...
* **MUST** contain the magic hex `0x414902` at offset 8 ([why?](https://github.com/AppImage/AppImageKit/issues/144))
* **MUST** contain the magic hex `0x41490201` at offset __TBD__ ([why?](https://github.com/AppImage/AppImageSpec/issues/38))
* The ELF part **MUST** be statically linked ([why?](https://github.com/AppImage/AppImageKit/issues/877))
* If the ELF part uses FUSE, it SHOULD try to be future-proof by not relying on hardcoded `/usr/bin/fusermount` or `/usr/bin/fusermount3` paths but also anticipate future paths of such helpers ([why?](https://github.com/AppImage/type2-runtime/issues/2))
